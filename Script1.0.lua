...
_G.SaveSetting = {
   ...
   RandomPos = true,
   AutoNewWorld = false,
   AutoBone = false--ถ้าเยอกว่านั้นใช้ ใส่ , นี้ด้วย เช่น
   --[[
   PiwwySave1 = false,
   PiwwySave12 = false
   ]]
   ...
   function LoadSettings()
	   if readfile and writefile and isfile and isfolder then
		   if not isfolder("Name Hub Premium Scripts") then
			   makefolder("Name Hub Premium Scripts")
		   end
		   if not isfolder("Name Hub Premium Scripts/Blox Fruits/") then
			   makefolder("Name Hub Premium Scripts/Blox Fruits/")
		   end
		   if not isfile("Name Hub Premium Scripts/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json") then
			   writefile("Name Hub Premium Scripts/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json", game:GetService("HttpService"):JSONEncode(_G.SaveSetting))
		   else
			   local Decode = game:GetService("HttpService"):JSONDecode(readfile("Name Hub Premium Scripts/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json"))
   ...
   function savesettings()
	   if readfile and writefile and isfile and isfolder then
		   if not isfile("Name Hub Premium Scripts/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json") then
			   ...
			   writefile("Name Hub Premium Scripts/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json", game:GetService("HttpService"):JSONEncode(Array))
		   end
	   else
		   return warn("Status : Undetected Executor")
	   end
...
function setwatermarktime()
    ...
    Library:SetWatermark("Name Hub | "..hr.." Hour(s) "..min.." Minute(s) "..sec.." Second(s)")
 end
...
local AutoMet = Tabs['Auto']:AddLeftTabbox()
...
local AutoMets = AutoMet:AddTab('Meterials')
...
spawn(function()
	while wait() do
        ...
        TP2(v.HumanoidRootPart.CFrame * CFrame.new(0,20,30))
        wait(.15)
        TP2(v.HumanoidRootPart.CFrame * CFrame.new(30,20,0))
      wait(.15)
        TP2(v.HumanoidRootPart.CFrame *CFrame.new(0,20,-30))
       wait(.15)
        TP2(v.HumanoidRootPart.CFrame * CFrame.new(-30,20,0))
...
while wait() do
    ...
    pcall(function()
        CheckQuest()
        if AutoFarm and (CFrameMon.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude > 1500 then
						TP3(CFrameMon)
					end
if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == false then
TP(CFrameMon)
if (CFrameMon.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 450 then
wait(.5)
        ...
        for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
            if v.Name == Name and v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") then
                ...
                repeat wait()
                    ...
                    Equip(WeaponName)
                    AutoHaki()
    if AutoFarm and (CFrameMon.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude > 275 then
        TP(CFrameMon)
				end
        TP2(v.HumanoidRootPart.CFrame * CFrame.new(0,20,30))
        wait(.15)
		...
		pcall(function()
			if Bringmob then
					if game.Players.LocalPlayer.PlayerGui.Main.Quest.Visible == true then
						for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
							if v.Name == (v.HumanoidRootPart.Position-_G.PosMon.Position).Magnitude <= 350 then
...
AutoMets:AddToggle('AutoBone', {
    Text = 'Auto Bones',
    Default = _G.SaveSetting.AutoBone,
    Tooltip = 'Auto Farm Bone',
})
Toggles.AutoBone:OnChanged(function(t)
    _G.SaveSetting.AutoBone = t
    AutoFarmBone = t
    Bringmob = t
    savesettings()
